...
# Longhorn Dashboard Port Forward Script
# Forwards the Longhorn UI service into localhost and opens it in the browser.
...
echo "Starting port-forward for Longhorn dashboard on $HOST:$PORT... (remote port $REMOTE_PORT)"
...
if kill -0 "$PORT_FORWARD_PID" 2>/dev/null; then
    ...
    echo "Opening Longhorn dashboard at http://$HOST:$PORT"
